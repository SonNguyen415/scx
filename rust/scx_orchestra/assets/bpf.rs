...
use scx_orchestra::{Domain, OrchestraClient, ShmQueue, Notification, SOCKET_PATH, SHM_BASE, ALLOCATOR};
pub use scx_orchestra::{QueuedTask, DispatchedTask};
use tarpc::{client::Config, context, tokio_serde::formats::Bincode};
...
impl ClientDomain {
    pub async fn register(name: &str) -> Self {
        let transport = tarpc::serde_transport::unix::connect(SOCKET_PATH, Bincode::default)
            .await.unwrap();
        let rpc = OrchestraClient::new(Config::default(), transport).spawn();
        rpc.register(context::current(), name.to_string()).await.unwrap().unwrap();
        ClientDomain {
            domain: Domain {
                name:           name.to_string(),
                cpu_ids:        vec![],
                client_count:   1,
                dispatch_queue: ShmQueue::open(&format!("{}-{}-dispatch", SHM_BASE, name)),
                task_queue:     ShmQueue::open(&format!("{}-{}-task",     SHM_BASE, name)),
                notif_queue:    ShmQueue::open(&format!("{}-{}-notif",    SHM_BASE, name)),
            },
            rpc,
        }
    }

    pub async fn request_cpu(&mut self, cpu_id: u32) -> Result<(), String> {
        self.rpc.request_cpu(context::current(), self.domain.name.clone(), cpu_id)
            .await.unwrap()
    }

    pub async fn release_cpu(&mut self, cpu_id: u32) -> Result<(), String> {
        self.rpc.release_cpu(context::current(), self.domain.name.clone(), cpu_id)
            .await.unwrap()
    }

    pub fn dispatch_task(&self, task: DispatchedTask) -> Result<(), i32> {
        match self.domain.dispatch_queue.enqueue(task) {
            true  => Ok(()),
            false => Err(-1),
        }
    }
    ...
    pub fn dequeue_task(&self) -> Option<QueuedTask> {
        self.domain.task_queue.dequeue()
    }

    pub fn poll_notifications(&self) -> Option<Notification> {
        self.domain.notif_queue.dequeue()
    }
}
...
impl Drop for ClientDomain {
    fn drop(&mut self) {
        ...
        tokio::runtime::Builder::new_current_thread()
            ...
            .unwrap()
            .block_on(async move {
                let _ = rpc.unregister(context::current(), name).await;
            });
    }
}
...
pub struct BpfScheduler<'cb> {
    ...
    domain: ClientDomain,                      // Orchestra domain associated with the scheduler
}
...
impl<'cb> BpfScheduler<'cb> {
    pub fn init(
        ...
        // Register to a domain
        let domain = tokio::runtime::Builder::new_current_thread()
                    .enable_all()
                    .build()
                    .unwrap()
                    .block_on(ClientDomain::register(name));
    ...
    pub fn dispatch_task(&mut self, task: &DispatchedTask) -> Result<(), libbpf_rs::Error> {
        ...
        *enq_cnt = task.enq_cnt;
        ...
        self.domain.dispatch_task(*task).expect("failed to enqueue task to the domain dispatch queue");
